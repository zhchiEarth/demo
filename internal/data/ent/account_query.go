...
func (aq *AccountQuery) QueryTokens() *AccountCTokenQuery {
	query := &AccountCTokenQuery{config: aq.config}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withTokens: aq.withTokens.Clone(),
...
func (aq *AccountQuery) WithTokens(opts ...func(*AccountCTokenQuery)) *AccountQuery {
	query := &AccountCTokenQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withTokens; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Tokens = []*AccountCToken{}
		}
		query.withFKs = true
		query.Where(predicate.AccountCToken(func(s *sql.Selector) {
			s.Where(sql.InValues(account.TokensColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_tokens
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_tokens" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_tokens" returned %v for node %v`, *fk, n.ID)
			}
